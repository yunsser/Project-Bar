			...
			<div name="addForm" id="form">
				<div class="mb-3">
					<label for="title">제목</label> <span type="text"
						class="form-control">${notice.title_no}</span>
				</div>
				...
				<div class="mb-3">
					...
					<label for="author">작성자</label> <span class="form-control"
						...
						name="master" value="${notice.master}"
						style="border: none; color: gray;" readonly>${notice.master}</span>
				</div>

				<div class="mb-3">
					<label for="content">내용</label> <span class="form-control"
						id="content_no">${notice.content_no}</span>
				</div>
						...
						<c:when test="${fn:length(notice.notice_attach)>0}">
							<c:forEach var="f" items="${notice.notice_attach}">
								...
								<div>
									...
									<a href="/bar/notice/file/download/${f.no_att_num}"><img src="/upload/${f.no_filename}" width="30%" alt="" class="thumb" style="display: block; margin: auto;"/></a>
									</span>
								</div>
							</c:forEach>
						</c:when>
			...
			<div class="btlistsav">
				<c:if test="${id eq board.author || id eq 'master'}">
					<button type="button" class="btn btn-sm btn-primary"
						onclick="location.href='/bar/notice/boardEdit?num=${notice.num_no}'">수정</button>
					<button type="button" class="btn btn-sm btn-primary"
						onclick="location.href=del_notice('${notice.num_no}')">삭제</button>
				</c:if>
