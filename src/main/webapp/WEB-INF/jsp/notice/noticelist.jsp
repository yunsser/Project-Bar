	...
	<table class="table" style="text-align: center;">
		...
		<c:forEach var="u" items="${viewAll}">
			...
			<tr>
				...
				<td>${u.num_no}</td>
				<td><a href="/bar/notice/boardDetail?num=${u.num_no}">${u.title_no}</a></td>
				<td>${u.master}</td>
				<td>${u.ndate}</td>
			</tr>
		</c:forEach>
		...
		<ul class="pagination" id="pagination">
			...
			<c:if test="${paging.startPage != 1 }">
				<a id="be" class="page-link"
					href="noticelist?nowPage=${paging.startPage - 1 }&cntPerPage=${paging.cntPerPage}">Before</a>
			</c:if>
				...
				<c:choose>
					...
					<c:when test="${p != paging.nowPage }">
						<li class="page-item"><span aria-hidden="true"><a
								class="page-link"
								href="noticelist?nowPage=${p }&cntPerPage=${paging.cntPerPage}">${p }</a></span></li>
					</c:when>
				</c:choose>
			</c:forEach>
