...
<!-- 헤더 -->
	...
	<article>
		...
		<form id = "updateForm"  action="/bar/notice/boardUpdate" enctype="multipart/form-data" onsubmit="return updateBoard();" >
		<input type="hidden" id="num_no" name="num_no" value="${notice.num_no}">
			<p>
			<div name="addForm" id="form">
			<div class="mb-3">
				<label for="title">제목</label><input type="text" id="title_no" name="title_no" class="form-control" value="${notice.title_no}">
			</div>
			...
			<div class="mb-3">
				...
				<label for="author">작성자</label> <span class="form-control"
					name="reg_id" id="reg_id"><input type="hidden" id="master" name="master"
					value="${notice.master}" style="border: none; color: gray;" readonly>${notice.master}</span>
			</div>

			<div class="mb-3">
				<label for="content">내용</label>
				<textarea class="form-control" id="content_no" name="content_no">${notice.content_no}</textarea>
			</div>

			<div> <!-- class="form-control" -->
				<label for="content">파일</label>
			<input type="file" id="files" name="files" class="form-control" accept="image/*" onchange="setThumbnail(event);">
				<c:choose>
					<c:when test="${fn:length(notice.notice_attach)>0}">
						<c:forEach var="f" items="${notice.notice_attach}">
							<fmt:formatNumber var="kilo" value="${f.no_filesize/1024}"
								maxFractionDigits="0" />
							<div id="${f.no_att_num}"><span class="form-control">
								<a href="/bar/notice/file/download/${f.no_att_num}">${f.no_filename}</a> [${kilo}kb]
								<img src="/upload/${f.no_filename}"  width="100px" height="100px"  alt="" class="thumb" />
								<a class="link_del" href="javascript:del_file(${f.no_att_num});">삭제</a></span>
							</div>
						</c:forEach>
					</c:when>
					<c:otherwise>
...
<!-- 수정 목록 버튼 -->
			<div class="btlistsav">
				...
				<button type="button" class="btn btn-sm btn-primary"
					onclick="location.href='/bar/notice/noticelist'">목록</button>
			</div>
	...
	<p class="text-center text-muted"
		...
		Yun Company, Inc</p>
		<script src="/js/notice/noticeedit.js"></script>
	<script
